...
class Utils():
    ...
    def get_server_data(self,mode:str="all")-> False or dict:
        ...
        try:
            if mode=="total":
                ...
                response = requests.get(url_server, headers=headers, timeout=timeout)
                return response.json()['data']
            elif mode=="all":
                response = requests.get(url_total, headers=headers, timeout=timeout)
                return response.json()['data'],self.get_server_data_team(1), self.get_server_data_team(2)
            elif mode=="status":
                response = requests.get(url_status, headers=headers, timeout=timeout)
                return response.json()['data']
            elif mode=="player":
                response = requests.get(url_player, headers=headers, timeout=timeout)
                return response.json()['data']
        except:
    ...
    def get_server_report_data(self,mode:str)->tuple or bool:
        ...
        try:
            if mode == 'kd':
                ...
                total_rank = int(sum([d.get('rank', 0) for d in total_data]) / len(total_data))
                team1_rank = int(sum([d.get('rank', 0) for d in team1_data]) / len(team1_data))
                team2_rank = int(sum([d.get('rank', 0) for d in team2_data]) / len(team2_data))
                return total_rank, team1_rank, team2_rank
            elif mode=="king":
                total_kill_max, total_kill_king = self.find_metrics_in_dict("king",total_data)
                team1_kill_max, team1_kill_king = self.find_metrics_in_dict("king",team1_data)
                team2_kill_max, team2_kill_king = self.find_metrics_in_dict("king",team2_data)
                ...
                hot_weapon = self.find_metrics_in_dict("hot",total_data, 'weaponS0')
                hot_grenade = self.find_metrics_in_dict("hot",total_data, 'weaponS6')
                hot_knife = self.find_metrics_in_dict("hot",total_data, 'weaponS7')
                return hot_weapon, hot_grenade, hot_knife
            elif mode=="num":
                return len(total_data), len(team1_data), len(team2_data)
            elif mode=="150":
                total_150 = self.find_metrics_in_dict("count",total_data, 'rank', 150)
                team1_150 = self.find_metrics_in_dict("count",team1_data, 'rank', 150)
                team2_150 = self.find_metrics_in_dict("count",team2_data, 'rank', 150)
                return total_150, team1_150, team2_150
            elif mode=="playerlist":
                total_list = self.find_metrics_in_dict("playerlist",total_data)
                team1_list = self.find_metrics_in_dict("playerlist",team1_data)
                team2_list = self.find_metrics_in_dict("playerlist",team2_data)
                return total_list, team1_list, team2_list
            elif mode=="info":
                server_info=self.get_server_data("server")
                servername = server_info['name'][:20] + '\n'
                mapmode = server_info['gameMode2'] + '-' +server_info['mapName2'] + '\n'
                team = "队伍1:" + server_info["team1"]['name'] + "  队伍2:" + server_info["team2"]['name'] + '\n'
    ...
    def get_server_report(self, mode:str="random")->bool or str: 
        try:
            ...
            total_kill, total_death, team1_kill, team1_death, team2_kill, team2_death = self.get_server_report_data("kd")
            total_rank, team1_rank, team2_rank =self.get_server_report_data("rank")
            total_kill_max, total_kill_king, total_death_max, total_death_king, team1_kill_max, team1_kill_king, team2_kill_max, team2_kill_king = self.get_server_report_data("king")
            hot_weapon, hot_grenade, hot_knife = self.get_server_report_data("hot")
            total_num, team1_num, team2_num = self.get_server_report_data("num")
            total_150, team1_150, team2_150 = self.get_server_report_data("150")
            ...
            if mode=="all":
                ...
                report.append(f"dsz report: 总击杀数为{total_kill}，总死亡数为{total_death}，队伍1击杀数为{team1_kill}，队伍1死亡数为{team1_death}，队伍2击杀数为{team2_kill}，队伍2死亡数为{team2_death}")
                report.append(f"dsz report: 平均等级为{total_rank}，队伍1平级为{team1_rank}，队伍2平均等级为{team2_rank}")
                report.append(f"dsz report: 击杀王为{total_kill_king}({total_kill_max})，狗带王为{total_death_king}({total_death_max})")
                report.append(f"dsz report: 队伍1击杀王为{team1_kill_king}({team1_kill_max})，队伍2击杀王为{team2_kill_king}({team2_kill_max})")
                ...
                report.append(f"dsz report: 当前服务器有{total_150}位150级玩家，队伍1有{team1_150}位，队伍2有{team2_150}位")
