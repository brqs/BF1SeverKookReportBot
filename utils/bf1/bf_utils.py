...
async def check_bind(kook: int) -> Union[dict, str, None]:
    """检查玩家是否绑定
    :param kook: 玩家kook
    :return: 返回玩家信息,如果未绑定则返回None,查询失败返回str信息"""
    player_pid = await BF1DB.get_pid_by_kook(kook)
    if not player_pid:
        return None
    # 修改逻辑,先从数据库中获取,如果数据库中没有则从API中获取
    player_info = await BF1DB.get_bf1account_by_pid(player_pid)
    if player_info:
        return {"displayName": player_info["display_name"], "pid": player_pid, "uid": player_info["uid"], "kook": kook}
    else:
        ...
        if isinstance(player_info, str):
            ...
            uid = player_info['result'][str(player_pid)]['nucleusId']
            return {"displayName": displayName, "pid": pid, "uid": uid, "kook": kook}
